...
from openpi import transforms  # 假设有一个 transforms 模块提供 pad_to_dim 等功能
from openpi.models import model as _model

def make_flexiv_example() -> dict:
    """Creates a random input example compatible with Flexiv config."""
    return {
        "state": np.ones((7,)),  # 匹配 observation.state
...
class FlexivInputs(transforms.DataTransformFn):
    ...
    def __call__(self, data: dict) -> dict:
        ...
        state = transforms.pad_to_dim(data["state"], self.action_dim)[:7]
