...
public class WorldAlign : MonoBehaviour
{
    ...
    public Vector3 GetPosition(Vector3 world)
    {
        return transform.InverseTransformPoint(world);
    }

    public Vector3 GetEuler(Vector3 world)
    {
        return Quaternion.Inverse(transform.rotation) * world;
    }

    public Quaternion GetRotation(Quaternion world)
    {
        return Quaternion.Inverse(transform.rotation) * world;
    }
    ...
    public Vector3 GetWorldEuler(Vector3 local)
    {
        return transform.rotation * local;
    }

    public Quaternion GetWorldRotation(Quaternion local)
    {
        return transform.rotation * local;
    }
}
